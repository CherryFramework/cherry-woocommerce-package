...
jQuery(document).ready(function($) {
	...
	$(document).on('reset_data', function(event) {
		...
		var item          = $('.product-large-image'),
			image         = $('img', item),
			initial_thumb = image.data('initial-thumb'),
			initial_large = image.data('initial-thumb-large');

		image.attr('src', initial_thumb).data('zoom-image', initial_large).attr('data-zoom-image', initial_large);
		zoomInit();
	});
